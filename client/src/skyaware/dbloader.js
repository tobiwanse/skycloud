...
import aircraft_types from './db/aircraft_types/icao_aircraft_types';
var _aircraft_cache = {};
...
function getIcaoAircraftTypeData(aircraftData, defer) {
    _aircraft_type_cache = aircraft_types;
    if (_aircraft_type_cache === null) {
        $.getJSON(aircraft_types)
...
function db_ajax(bkey) {
        ...
        if (_request_count < MAX_REQUESTS) {
                ...
                ++_request_count;
                defer = _request_cache[bkey] = $.ajax({ url: '/api/skyaware/db/' + bkey,
                                                        cache: true,
                                                        timeout: 5000,
                                                        dataType : 'json' });
...
function db_ajax_request_complete() {
        ...
        if (_request_queue.length == 0) {
                ...
                ajaxreq = $.ajax({ url: '/api/skyaware/db/' + req.bkey,
                                   cache: true,
                                   timeout: 5000,
                                   dataType : 'json' });
