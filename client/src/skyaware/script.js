...
import './config';
import "./style.css";
import { get_unit_label, format_altitude_long, format_onground, format_speed_long, format_vert_rate_long, format_track_long, format_latlng, format_distance_long, format_nac_p, format_nac_v, format_distance_short } from './formatter';
...
//var OLMap = 'sdfsdfs';
var StaticFeatures = new Collection( );
var SiteCircleFeatures = new Collection( );
PlaneIconFeatures = new Collection( );
PlaneTrailFeatures = new Collection( );
...
function processReceiverUpdate ( data, receiver_source ) {
   ...
   if ( receiver_source === "skyaware978" ) {
      ...
      if ( MessageCountHistory_UAT.length > 0 && MessageCountHistory_UAT[ MessageCountHistory_UAT.length - 1 ].messages > data.messages ) {
         MessageCountHistory_UAT = [ {
            'time': MessageCountHistory_UAT[ MessageCountHistory_UAT.length - 1 ].time,
            'messages': 0
         } ];
      }
...
function fetchData ( ) {
   ...
   if ( UAT_Enabled ) {
      ...
      FetchPending_UAT = $.ajax( {
         url: 'data-978/aircraft.json',
         timeout: 5000,
         cache: false,
         dataType: 'json'
      } );
...
function start_load_history ( map ) {
   PlaneFilter.ADSB = true
   PlaneFilter.MLAT = true
   PlaneFilter.Other = true
   PlaneFilter.TISB = true
   PlaneFilter.UAT = true
...
function CreateACLayers ( map ) {
   ...
   const site_pos_layer = new VectorLayer( {
      name: 'site_pos',
      type: 'overlay',
   ...
   const ac_trails_layer = new VectorLayer( {
      name: 'ac_trail',
      type: 'overlay',
   ...
   OverLayerCollection.push( iconsLayer );
   OverLayerCollection.push( site_pos_layer );
   OverLayerCollection.push( ac_trails_layer );

   OLMap.on( [ 'click', 'dblclick' ], function ( evt ) {
      var hex = evt.map.forEachFeatureAtPixel( evt.pixel, function ( feature, layer ) {
         return feature.hex;
      }, {
...
function selectPlaneByReg ( reg, autofollow ) {
   // If SelectedPlane has something in it, clear out the selected
   if ( SelectedAllPlanes ) {
      deselectAllPlanes( );
   }
...
export { init_skyaware, start_load_history, CreateACLayers, selectAllPlanes, selectPlaneByHex, refreshSelected };