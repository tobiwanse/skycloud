"use strict";
import { refreshSelected, selectPlaneByHex } from './script';
import { findICAORange } from './flags';
import { getAircraftData } from './dbloader';
import registration_from_hexid from './registrations';
import { getBaseMarker, svgPathToURI } from './markers';
import { get_unit_label, format_altitude_long, format_onground, format_speed_long, format_vert_rate_long, format_track_long, format_latlng, format_distance_long, format_nac_p, format_nac_v, format_distance_short, convert_altitude, convert_speed } from './formatter';
...
PlaneObject.prototype.updateTrack = function(receiver_timestamp, last_timestamp) {
        ...
        var projHere = Proj.fromLonLat(this.position);
        var projPrev;
        var prev_time;
        if (this.prev_position === null) {
                ...
                projPrev = Proj.fromLonLat(this.prev_position);
        ...
        if (this.track_linesegs.length == 0) {
                ...
                var newseg = { fixed: new LineString([projHere]),
                               feature: null,
        ...
        if (est_track) {
                ...
                if (!lastseg.estimated) {
                        ...
                        this.track_linesegs.push({ fixed: new LineString([projPrev]),
                                                   feature: null,
        ...
        if (lastseg.estimated) {
                ...
                // solid lines.
                lastseg.fixed.appendCoordinate(projPrev);
                lastseg = { fixed: new LineString([projPrev]),
                            feature: null,
                            update_time: prev_time,
                            estimated: false,
                            ground: (this.altitude === "ground"),
...
PlaneObject.prototype.updateIcon = function() {
        ...
        if (this.markerStyle === null || this.markerIcon === null || this.markerSvgKey != svgKey) {
                ...
                if (AircraftLabels && this.flight != null) {
                        this.markerStyle = new Style({
                                ...
                                text: new Text({
                                        ...
                                        backgroundFill: new Stroke({color: 'rgba(0, 47, 93, 0.8'}),
                                        textAlign: 'center',
                                        offsetY: -20,
                                        font: '10px Helvetica',
                                        padding: [1,0,0,2]
                ...
                this.markerStaticIcon = null;
                this.markerStaticStyle = new Style({});
...
PlaneObject.prototype.updateData = function(receiver_timestamp, data, receiver_source) {
        ...
        if ('lat' in data && 'lon' in data) {
                ...
                if (SitePosition !== null) {
                        this.sitedist = Sphere.getDistance(SitePosition, this.position);
                }
...
PlaneObject.prototype.updateMarker = function(moved) {
        ...
        if (this.marker) {
                if (moved) {
                        this.marker.setGeometry(new Point(Proj.fromLonLat(this.position)));
                        this.markerStatic.setGeometry(new Point(Proj.fromLonLat(this.position)));
                }
        } else {
                this.marker = new Feature(new Point(Proj.fromLonLat(this.position)));
                this.marker.hex = this.icao;
                this.marker.setStyle(this.markerStyle);
                PlaneIconFeatures.push(this.marker);

                this.markerStatic = new Feature(new Point(Proj.fromLonLat(this.position)));
...
PlaneObject.prototype.updateLines = function() {
        ...
        var geom = new LineString([lastfixed, Proj.fromLonLat(this.position)]);
        this.elastic_feature = new Feature(geom);
        if (lastseg.estimated) {
        ...
        for (var i = 0; i < this.track_linesegs.length; ++i) {
                var seg = this.track_linesegs[i];
                if (seg.feature === null) {
                        seg.feature = new Feature(seg.fixed);
                        if (seg.estimated) {
